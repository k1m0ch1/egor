  ...
  @for($j = 1; $j<=$h; $j++)
    <td class='pindah' style='padding-left: 10px; padding-bottom: 10px;'>
      ...
      @if ($checkFile[0] != 'HTTP/1.1 404 Not Found')
          <img class="img-thumbnail" src="{{ asset('assets/img/uploaded') }}/{{ $img[$a][3] }}" id="img-thumbnail-{{$a}}" alt="" />
      @elseif ($img[$a][3] == 'holder.js/180x180')
          <img data-src="{{ $img[$a][3] }}" class="img-thumbnail" id="img-thumbnail-{{$a}}" alt="" />
      @else
        <img id='gambar' class="img-thumbnail" src="{{ asset('assets/img') }}/addimage.png" id="img-thumbnail-{{$a}}" alt="" />
      @endif
      <a href="{{ $img[$a][2] }}">{{ $img[$a][0] }} </a>
      <div id='minimenu' style="position: absolute">
        <button id='editGrid-{{ $a }}' class="fa fa-edit" value="{{$a}}" style='float: right; padding-top: 5px;'></button>
